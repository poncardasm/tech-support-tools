"""Main entry point for log-parse CLI on Windows."""
...
@click.option(
    "--format",
    "-f",
    help="Force log format (evtx, evtx_xml, json_lines, iis, syslog, plain)",
)
@click.option(
...
def cli(
    ...
    Examples:
        log-parse application.evtx
        log-parse --level ERROR,WARN system.log
    ...
    stats = {
        ...
        "errors": sum(1 for e in filtered_entries if e.level in ["ERROR", "CRITICAL"]),
