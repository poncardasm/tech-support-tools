...
- **Language:** Python 3.10+ (runs on Windows via Python installer or winget)
- **CLI framework:** Click
- **Config parsing:** PyYAML
- **Local LLM:** `requests` library to Ollama at `http://localhost:11434` (optional, only if `--llm` used)
- **Packaging:** `pip install -e .` or `.exe` via PyInstaller
- **Config:** `%APPDATA%\ticket-triage\rules.yaml` for custom rules
...
│   └── test_triage.py
├── build/
│   └── ticket-triage.exe (PyInstaller output)
├── .gitignore
└── README.md
```
...
- [ ] `Get-Content ticket.txt | ticket-triage` produces valid output in < 500ms
...
- [ ] Config override works from `%APPDATA%`
