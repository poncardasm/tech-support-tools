# Ticket Triage CLI — Implementation Plan (Windows)
...
pip install build pyyaml click pyinstaller requests
```
...
Load with `yaml.safe_load()`. Merge with user config at `%APPDATA%\ticket-triage\rules.yaml` if exists.
...
### 7b. Build standalone executable

```powershell
pip install pyinstaller
pyinstaller ticket-triage.spec
# Output: dist/ticket-triage.exe
```

### 7c. PyInstaller spec (ticket-triage.spec)

```python
a = Analysis(
    ['ticket_triage/__main__.py'],
    pathex=[],
    binaries=[],
    datas=[('ticket_triage/rules.yaml', 'ticket_triage')],
    hiddenimports=['click'],
    hookspath=[],
    runtime_hooks=[],
    excludes=[],
...
2. **Path handling** — Use `pathlib.Path` for cross-platform paths. Avoid hardcoded backslashes.
3. **Unicode in output** — Windows Terminal and PowerShell 7+ handle emoji. Test on legacy cmd.exe.
4. **YAML anchor refs** — Don't use YAML anchors in rules.yaml with `pyyaml` default loader; use `yaml.safe_load()` which ignores them.
5. **LLM timeout** — Set `timeout=30` on Ollama requests. Never hang the CLI waiting for LLM.
6. **PowerShell pipes** — Test `Get-Content ticket.txt | ticket-triage` explicitly.
